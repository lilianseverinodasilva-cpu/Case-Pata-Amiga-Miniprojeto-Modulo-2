...
FROM stg_pedido
WHERE CategoriaProduto IS NOT NULL
...
FROM stg_loja_praca
WHERE CodPraca IS NOT NULL AND TRIM(CodPraca) <> ''
GROUP BY CodPraca;
...
SELECT 
    ...
    dp.sk_praca,
    CAST(REPLACE(REPLACE(p.`PercentualPublico`, '%', ''), ',', '.') AS DECIMAL(6,4)) AS fator_publico
FROM stg_loja_praca p
JOIN dim_praca dp 
  ON p.`CodPraca` = dp.cod_praca;
-- =====================================================================================
